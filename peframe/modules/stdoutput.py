...
def show_auto(info, cert_dump, peid, antidbg, antivm, xor, apialert,
              sectionsalert, fileurl, meta):
    #INFO
    ...
    directory = short_info[
        "Directories"] # contains a list -> ', '.join(directory)
        ...
        for sign in detected:
            if sign == "Sign":
                ...
                if cert_matched:
                    ...
                    print "Block Size".ljust(18), str(
                        cert_matched["Block Size"]) + " byte"
                    print "Hash MD5".ljust(18), str(cert_matched["Hash MD5"])
                    print "Hash SHA-1".ljust(18), str(
                        cert_matched["Hash SHA-1"])
    ...
    if "Xor" in detected:
        print "\nXOR discovered"
        print "-" * 60
        print "Key length".ljust(18), "Offset (hex)".ljust(
            18), "Offset (dec)".ljust(18)
    ...
    if apialert:
        ...
        if apialert_matched:
            print "\nSuspicious API discovered [" + str(
                len(apialert_matched)) + "]"
            print "-" * 60
    ...
    if sectionsalert:
        ...
        if sectionsalert_matched:
            print "\nSuspicious Sections discovered [" + str(
                len(sectionsalert_matched)) + "]"
            print "-" * 60

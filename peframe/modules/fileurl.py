...
def get(filename):
    ...
    for found_str in get_process(PEtoStr):
        fname = re.findall("(.+\.([a-z]{2,3}$))+", found_str,
    ...
    for elem in sorted(set(array)):
        match = re.search(
            "^http:|^ftp:|^sftp:|^ssh:|^www|.com$|.org$|.it$|.co.uk$"
            "|.ru$|.jp$|.net$|.ly$|.gl$|^([0-9]{1,3})(?:\.[0-9]{1,3}){3}$",
            elem, re.IGNORECASE)
    ...
    if arrayFileNames:
        ...
        uniq_descr = []
        [item for item in arrayFileNames if
         item[0] not in uniq_descr and not uniq_descr.append(item[0])]
